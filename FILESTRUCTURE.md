...
* Python source files:
  * The Python package source files are located in the `latit_space` directory.
  * The file `latit_space.__main__.py` defines a command line interface that
    is accessible both as the command `latit_space` and via
    `python -m latit_space`.
  * `tests/test_cli.py` contains rudimentary testing of this CLI.
  * `tests/test_latit_space.py` contains the unit tests for the package.
  * `tests/conftest.py` contains testing setup and configuration for `pytest`
  * The `notebooks` directory contains an example Jupyter notebook on how to use `latit_space`.
